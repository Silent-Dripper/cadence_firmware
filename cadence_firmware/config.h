...
// Set this contstant to tell the firmware what type of drivers/other hardware are attached to the Arduino.
#define PLATFORM PLATFORM_SILENT_DRIPPER_PCB

// Defines if the given actuator will be controlled via a PC over the serial port. If false, the actuator will respond to it's corresponding fingerprint sensor.
...
#define AC_MOTOR_SHIELD 1  // Use the Adafruit Motor Shield v2 (https://www.adafruit.com/product/1438) to drive the actuators.
#define AC_TMC2208 2 // Use the TMC2208s on the Silent Dripper PCB to drive the Stepper Motors, the MaschinenReich XP88-ST01.
...
// The Cadence PCB is capable of driving either a motor OR a solenoid, and is the only platform that supports both types of actuators.
// If you want to drive a solenoid, set these variables to `false`.
#define ACTUATOR_1_MOTOR true
#define ACTUATOR_2_MOTOR true
...
// If you change the step frequency of the TMC's, and it looks like the drips are getting cut off, start here.
